...
class PackageDetailsRepositoryImpl(
    ...
    private suspend fun fetchPackageDetails(packageName: String, deviceId: String): PackageDetails? {
        return try {
            val dumpsysResult =
    ...
    private fun extractServices(output: String, packageName: String): List<PackageDetails.ServiceInfo> {
        ...
        ParsePatterns.SERVICE_PATTERN_1.findAll(output).forEach { match ->
            ...
            if (fullName.startsWith(packageName)) {
                ...
                services.add(
                    PackageDetails.ServiceInfo(name = fullName, exported = exported, enabled = enabled)
                )
            }
        }

        if (services.isEmpty()) {
            ParsePatterns.SERVICE_PATTERN_2.findAll(output).forEach { match ->
                val fullName = "${match.groupValues[1]}/${match.groupValues[2]}"
                if (fullName.startsWith(packageName)) {
                    val contextStart = (match.range.first - 300).coerceAtLeast(0)
                    val contextEnd = (match.range.last + 300).coerceAtMost(output.length)
    ...
    private fun extractReceivers(output: String, packageName: String): List<PackageDetails.ReceiverInfo> {
        ...
        if (receiverSection.isNotEmpty()) {
            ParsePatterns.RESOLVER_TABLE_COMPONENT.findAll(receiverSection).forEach { match ->
                ...
                if (pkg == packageName) {
                    receivers.add(
                        PackageDetails.ReceiverInfo(name = "$pkg/$className", exported = true, enabled = true)
                    )
                }
            }
        }
        ...
        ParsePatterns.RECEIVER_PATTERN_1.findAll(output).forEach { match ->
            val fullName = "${match.groupValues[1]}/${match.groupValues[2]}"
            if (fullName.startsWith(packageName)) {
                val contextStart = (match.range.first - 300).coerceAtLeast(0)
                val contextEnd = (match.range.last + 300).coerceAtMost(output.length)
                ...
                val enabled = !(ParsePatterns.ENABLED_FALSE in context || ParsePatterns.ENABLED_FALSE_ALT in context)
                receivers.add(
                    PackageDetails.ReceiverInfo(name = fullName, exported = exported, enabled = enabled)
                )
            }
        }

        if (receivers.isEmpty()) {
            ParsePatterns.RECEIVER_PATTERN_2.findAll(output).forEach { match ->
                val fullName = "${match.groupValues[1]}/${match.groupValues[2]}"
                if (fullName.startsWith(packageName)) {
                    val contextStart = (match.range.first - 300).coerceAtLeast(0)
                    val contextEnd = (match.range.last + 300).coerceAtMost(output.length)
    ...
    private fun extractContentProviders(output: String, packageName: String): List<PackageDetails.ProviderInfo> {
        ...
        if (providerSection.isNotEmpty()) {
            ParsePatterns.REGISTERED_PROVIDER_LINE.findAll(providerSection).forEach { match ->
                ...
                if (pkg == packageName) {
                    providers.add(
                        PackageDetails.ProviderInfo(name = "$pkg/$className", exported = true, enabled = true)
                    )
                }
            }
        }
        ...
        ParsePatterns.PROVIDER_PATTERN_1.findAll(output).forEach { match ->
            ...
            if (fullName.startsWith(packageName)) {
                ...
                val enabled = !(ParsePatterns.ENABLED_FALSE in context || ParsePatterns.ENABLED_FALSE_ALT in context)
                providers.add(
                    PackageDetails.ProviderInfo(name = fullName, exported = exported, enabled = enabled)
                )
            }
        }
